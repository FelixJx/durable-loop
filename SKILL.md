...
- **人工审核反馈回流（Gap2 2026-06-25）** → `python scripts/durable_loop_review.py --feature F [--project-dir D] --decision approve|reject|modify --reason "人类理由" [--draft <path>]`。把一次性的人工审核裁决沉淀成可 search 的 learning：`approve`→pattern(conf 7)、`reject`→pitfall(conf 8)、`modify`→同时记一条 pattern(`[修改自审核]`，conf 6)+一条 pitfall(被要求改掉的部分，conf 8)。key=`review-<decision>-<slug>`（slug 取自 `--draft` 文件名或 feature+时间 hash），同 (feature,decision,draft) 复审自动合并而非堆行；source 为 `--draft` 路径（让 `prune` 后续可据失效标 stale）否则 `human-review`。同时 append 一行到 `session.log`（tool=`durable_loop_review`）。**只读 reason/draft、不碰 `pending_approval.json`**（与 HITL 恢复解耦）；直接 import `durable_loop_learn` 的纯函数复用其 log/去重/原子写机制；全程 fail-open exit 0，仅非法 feature/缺 project_dir/空 reason 退 2。下次 reflect-in `durable_loop_learn.py search <feature> --query '...'` 即复用/规避
...
- **FAIL 三维度归因（#3 2026-06-23）** → `python scripts/diagnose.py <feature> [project_dir] [--limit 5]`（只读 reporter：读 `checkpoint.verify_history` 最近 N 条，按启发式分流到 **agent**（晚期 iter 间歇 FAIL+有 PASS→换模型/强化 driver prompt）/ **harness**（history<3 或 FAIL 后无 PASS→查 Stop hook/reset_every_n/调度间隔 z.ai 用 240s）/ **skill**（早期 iter<5 FAIL 密集→查 done.criteria/learnings）三轴，给"优先改哪层"+pass-rate/streak/trend，并明确警告"不要无脑往 learnings/SKILL.md 堆规则"。无 checkpoint/全 PASS→exit 0；纯只读不写文件、不拦截）
